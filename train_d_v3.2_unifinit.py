...
from discriminator_nonorm import discriminator2 as discriminator
...
with tf.variable_scope("rnn_module1"):
   ...
   var_dict['dfcW1'] = tf.Variable(tf.random_uniform([5096, 1024], -0.08,0.08), name='dfcW1')
   var_dict['dfcb1'] = tf.Variable(tf.random_uniform([1024], -0.08,0.08), name='dfcb1')
   #var_dict['dbeta1'] = tf.Variable(tf.zeros([2048]), name='dbeta1')
   #var_dict['dscale1'] = tf.Variable(tf.ones([2048]), name='dscale1')
   var_dict['dfcW2'] = tf.Variable(tf.random_uniform([1024, 128], -0.08,0.08), name='dfcW2')
   var_dict['dfcb2'] = tf.Variable(tf.random_uniform([128], -0.08,0.08), name='dfcb2')
   #var_dict['dbeta2'] = tf.Variable(tf.zeros([1024]), name='dbeta2')
   #var_dict['dscale2'] = tf.Variable(tf.ones([1024]), name='dscale2')
   var_dict['dfcW3'] = tf.Variable(tf.random_uniform([128, 1], -0.08,0.08), name='dfcW3')
   var_dict['dfcb3'] = tf.Variable(tf.random_uniform([1], -0.08,0.08), name='dfcb3')
   ...
   s_r = discriminator(features, answers_true_n, var_dict)
   s_w = discriminator(features_false, answers_true_n, var_dict)
   s_f = discriminator(features, g_true_n, var_dict) #g_true
...
for train_loops in range(10):
   ...
   while batch_no*batch_size < len(qa_data['training']) - batch_size:
      ...
      if batch_no < 2000:
         lossg_val, g_out = sess.run([lossg, scores_emb], feed_dict={
            noise: noise_in,
            answers_true: answer_in_true,
            answers_false: answer_in_false,
            image_false: im_feat_false,
      ...
      np.save('loss_vals_d_v' + save_ver, loss_vals)
      np.save('lossg_vals_d_v' + save_ver, lossg_vals)
