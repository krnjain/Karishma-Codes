...
def combine_embeddings(cnn, rnn, var_dict):
    '''
    ...
    '''
    cnn_m = tf.add(tf.matmul(cnn, var_dict['cemcnnfcW1']), var_dict['cemcnnfcb1'])
    cnn_mt = tf.tanh(cnn_m)
    rnn_m = tf.add(tf.matmul(rnn, var_dict['cemrnnfcW1']), var_dict['cemrnnfcb1'])
    rnn_mt = tf.tanh(rnn_m)

    cnn_a = tf.add(tf.matmul(cnn, var_dict['ceacnnfcW1']), var_dict['ceacnnfcb1'])
    cnn_at = tf.tanh(cnn_a)
    rnn_a = tf.add(tf.matmul(rnn, var_dict['cearnnfcW1']), var_dict['cearnnfcb1'])
    rnn_at = tf.tanh(rnn_a)

    multiplied = tf.multiply(cnn_mt,rnn_mt)
    added = tf.add(cnn_at,rnn_at)

    x = tf.concat([multiplied, added], 1)
    xt = tf.tanh(x)
    return xt


def generator_me(features, var_dict, keep_prob=0.85):
    '''
    ...
    fc1_do = tf.nn.dropout(fc1, keep_prob=keep_prob)
    ...
    fc2 = tf.nn.relu_layer(fc1_do, var_dict['gfcW2'], var_dict['gfcb2'])
    fc2_do = tf.nn.dropout(fc2, keep_prob=keep_prob)
    ...
    fc3 = tf.nn.relu_layer(fc2_do, var_dict['gfcW3'], var_dict['gfcb3'])
    fc3_do = tf.nn.dropout(fc3, keep_prob=keep_prob)
    ...
    fc4 = tf.nn.xw_plus_b(fc3_do, var_dict['gfcW4'], var_dict['gfcb4'])
