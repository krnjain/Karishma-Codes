...
rnn_out = tf.expand_dims(rnn_out_true_n, 1)
rnn_out = tf.expand_dims(rnn_out, 1)
rnn_out = tf.tile(rnn_out, [1,7,7,1])

features = tf.concat([rnn_out, cnn_out_true_n], 3)


conv1W = tf.Variable(tf.truncated_normal([7,7,4096,2048]))
conv1b = tf.Variable(tf.constant(0.1, shape=[2048]))
conv1_out = tf.nn.conv2d(features, conv1W, strides=[1,1,1,1], padding='SAME')
conv1_bout = tf.nn.bias_add(conv1_out, conv1b)
conv6_rout = tf.nn.relu(conv1_bout)
conv6_dropout = tf.nn.dropout(conv6_rout, keep_prob=0.9)
