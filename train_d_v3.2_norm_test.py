...
# gradient clipping
gvs_d = opt.compute_gradients(loss,d_vars)

#for i in range(len(tvars)):
#   print(tvars[i].name)
clipped_gvs_d = [(tf.clip_by_value(grad, -10.0, 10.0), var) for grad, var in gvs_d]
train_op_d = opt.apply_gradients(clipped_gvs_d)


gvs_g = opt.compute_gradients(loss,g_vars)

#for i in range(len(tvars)):
#   print(tvars[i].name)
clipped_gvs_g = [(tf.clip_by_value(grad, -10.0, 10.0), var) for grad, var in gvs_g]
train_op_g = opt.apply_gradients(clipped_gvs_g)
...
for train_loops in range(10):
   ...
   while batch_no*batch_size < len(qa_data['training']):
      ...
      _, _, loss_val, g_out, cmean = sess.run([train_op_g, train_op_d, loss, scores_emb, cnn_mean], feed_dict={
         noise: noise_in,
