...
#    var_dict['dfcW2'] = tf.Variable(tf.truncated_normal([2048, 1024]), name='dfcW2')
#    var_dict['dfcb2'] = tf.Variable(tf.constant(0.1, shape=[1024]), name='dfcb2')
#    var_dict['dbeta2'] = tf.Variable(tf.zeros([1024]), name='dbeta2')
#    var_dict['dscale2'] = tf.Variable(tf.ones([1024]), name='dscale2')
#    var_dict['dfcW3'] = tf.Variable(tf.truncated_normal([1024, 512]), name='dfcW3')
...
#    s_r, fc6, fc5n, fc4n = discriminator(tf.concat([state_emb,image_emb], 1), answers_true, var_dict)
#    s_w, fc6, fc5n, fc4n = discriminator(tf.concat([state_emb2,image_emb2], 1), answers_true, var_dict)
#    s_f, fc6, fc5n, fc4n = discriminator(tf.concat([state_emb,image_emb], 1), scores_emb, var_dict) #g_true
...
saver = tf.train.Saver()
saver.restore(sess, "./train_d_simple_v3")
...
start = time.time()
(questions_in, im_feat, im_ids, question_ids, answers, qtext) = get_validation_batch_ans(batch_no, batch_size, qa_data)
...
correct_ans = (answers_idx==answers)
print(g_out)
print(answers_idx)
print(answers)
print(correct_ans)
...
for i in range(batch_size):
   if correct_ans[i]:
      ...
      while len(imid) < 6:
         imid = '0' + imid
      os.system('cp ./Images/mscoco/val2014/COCO_val2014_000000' + imid + '.jpg ./saved_images/COCO_val2014_000000' + imid + '.jpg')
      print('imid = ' + imid + ', question = ' + qtext[i] + ', answer generated = ' + answers_out[i]['answer'])
