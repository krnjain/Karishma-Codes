...
for eidx in range(20):
   ...
   while batch_no*batch_size < len(qa_data['training']) - batch_size:
      ...
      _, loss_val, = sess.run([train_op_d, loss], feed_dict={
         ...
         answers_f: answer_in_false,
         image_f: im_feat_false,
         question_f: questions_in_false 
      })

      _, lossg_val, g_out = sess.run([train_op_g, loss2, scores_emb], feed_dict={
         answers_true: answer_in_true,
         image: im_feat_true,
         question: questions_in_true,
         answers_f: answer_in_false,
      ...
      np.save('acc_vals_d_simple_v' + save_ver, acc_vals)
      ...
      if batch_no % 25 == 0:
         ...
         saver.save(sess, 'train_d_simple_v' + save_ver)
         #weights_save = {}
         #for i in range(len(tvars)):
